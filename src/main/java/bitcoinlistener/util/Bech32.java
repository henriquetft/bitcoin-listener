...
public class Bech32 {
    ...
    public static Bech32Data decode(final String str) throws RuntimeException {
        ...
        for (int i = 0; i < str.length(); ++i) {
            char c = str.charAt(i);
            if (c < 33 || c > 126) throw new RuntimeException("Invalid character " + c + " at position " + i);
            if (c >= 'a' && c <= 'z') {
                if (upper)
                    throw new RuntimeException("Invalid character " + c + " at position " + i);
                lower = true;
            }
            if (c >= 'A' && c <= 'Z') {
                if (lower)
        ...
        if (dataPartLength < 6) throw new RuntimeException("Data part too short: " + dataPartLength);
        byte[] values = new byte[dataPartLength];
        for (int i = 0; i < dataPartLength; ++i) {
            char c = str.charAt(i + pos + 1);
            if (CHARSET_REV[c] == -1) throw new RuntimeException("Invalid character " + c + " at position " + (i + pos + 1));
            values[i] = CHARSET_REV[c];
        }
        String hrp = str.substring(0, pos).toLowerCase(Locale.ROOT);
        if (!verifyChecksum(hrp, values)) throw new RuntimeException("Invalid checksum");
